...
let walkAngle = 0 // Переменная для отслеживания угла поворота
let isEating = false
...
function startPositionUpdates() {
    if (positionInterval) clearInterval(positionInterval)
        ...
        // Сохраняем центральную точку, вокруг которой будем ходить
        const centerX = posX
        const centerZ = posZ
        const radius = 1.0
        ...
        positionInterval = setInterval(() => {
            ...
            // Увеличиваем угол (0.1 — скорость ходьбы, можно подправить)
            walkAngle += 0.1
            if (walkAngle > Math.PI * 2) walkAngle = 0

                // Вычисляем новые координаты круга
                const newX = centerX + Math.cos(walkAngle) * radius
                const newZ = centerZ + Math.sin(walkAngle) * radius

                // Заставляем бота смотреть по направлению движения (опционально)
                // yaw = (walkAngle * 180 / Math.PI) + 90

                const buf = Buffer.alloc(33)
                buf.writeDoubleBE(newX, 0)
                buf.writeDoubleBE(posY, 8) // Y оставляем прежним
                buf.writeDoubleBE(newZ, 16)
                buf.writeFloatBE(yaw, 24)
                buf.writeFloatBE(pitch, 28)
                buf.writeUInt8(1, 32) // OnGround = true

                sendPlayPacket(0x15, buf) // Player Position And Rotation

                // Обновляем текущие переменные, чтобы другие функции знали, где мы
                posX = newX
                posZ = newZ
        }, 50) // 50мс = 1 тик в Minecraft (20 раз в секунду)
        console.log('[POS] Started walking in circle (R=1)')
}
...
// ===== жирмаксинг =====
function eatItem() {
    if (isEating || !sock || !isRunning) return;

    isEating = true;
    log('🍔 Проголодался, начинаю кушать...');

    // Пакет Use Item (ID 0x32 для 1.20.1 Play)
    // 0 = Main hand (основная рука)
    // Далее идет VarInt Sequence (обычно 0)
    sendPlayPacket(0x32, writeVarInt(0), writeVarInt(0));

    // Еда в майнкрафте естся 1.61 секунды (32 тика).
    // Ставим таймер на 2 секунды, чтобы наверняка.
    setTimeout(() => {
        // Чтобы "отпустить" еду, в некоторых случаях нужно отправить пакет смены слота
        // или просто дождаться обновления состояния голода от сервера.
        // Minecraft 1.20.1 часто требует Player Action (Finish eating)

        const finishEating = Buffer.concat([
            writeVarInt(5), // Action: Finish eating/Release use item
                                           writeVarInt(0), // Block Position X (0 для еды)
        writeVarInt(0), // Y
                                           writeVarInt(0), // Z
                                           Buffer.from([0x00]) // Face
        ]);
        sendPlayPacket(0x21, finishEating); // Player Action (0x21)
...
function handlePlayPacket(pkt) {
    ...
    // Disconnect (Play state)
    if (id === 0x1A) {
        const reason = readString(pkt, o)
        log(`❌ Кик из игры: ${reason ? reason.value : 'unknown'}`)
        if (sock) sock.destroy() // Это триггерит sock.on('close')
            return
    }
    ...
    // Health & Hunger (0x57 для 1.20.1)
    if (id === 0x57 || id === 0x1F || id === 0x49) {
        if (pkt.length - o < 4) return;
        ...
        console.log(`[STATUS] HP:${health.toFixed(1)} Food:${food}`);

        // Логика авто-еды
        if (food <= 14 && !isEating && isRunning) {
            eatItem();
        }
...
function connect() {
    ...
    sock.on('error', (e) => {
        ...
        // Здесь не вызываем connect(), так как после error всегда вызывается close
    })
    ...
    sock.on('close', () => {
        ...
        log(`🔌 Соединение полностью закрыто. Реконнект через 10 секунд...`)

        // Планируем перезаход
        if (!reconnectTimeout) {
            reconnectTimeout = setTimeout(() => {
                connect();
            }, 1000); // 10 секунд пауза перед перезаходом
        }
    })
