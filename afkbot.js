...
function sendCommand(cmd) {
    if (!sock || !isRunning) {
        console.log('[CMD] Cannot send - not connected')
        return false
    }
...
function startPositionUpdates() {
    ...
    }, 1000) // каждую секунду вместо 50ms
...
function handleLoginPacket(pkt) {
    ...
    if (id === 0x02) {
        ...
        gamePhase = 'play'
        isRunning = true
        log('✅ Бот зашёл на сервер!\n\n⏳ Жду сообщение с кодом верификации...')
...
function handlePlayPacket(pkt) {
    ...
    // System Chat - ВАЖНО для верификации!
    if (id === 0x60 || id === 0x64 || id === 0x5F || id === 0x67) {
        ...
        if (msg) {
            ...
            // Проверяем на запрос верификации
            if (msgText.includes('/verify')) {
                // Извлекаем код из сообщения
                const match = msgText.match(/\/verify\s+([A-Z0-9]+)/i)
                if (match) {
                    const code = match[1].toUpperCase()
                    log(`🔐 Получен код верификации: ${code}\n\nАвтоматически отправляю...`)
                    
                    // Отправляем команду верификации
                    setTimeout(() => {
    ...
    if (id === 0x3C || id === 0x38) {
        ...
        if (teleportId) {
            // Отправляем вручную без sendPlayPacket
            const tidBuf = writeVarInt(teleportId.value)
            const body = Buffer.concat([Buffer.from([0x00]), tidBuf])  // pktId=0x00 + teleportId
            const inner = Buffer.concat([writeVarInt(0), body])        // dataLen=0 (несжатый)
            console.log('[POS] full packet hex:', Buffer.concat([writeVarInt(inner.length), inner]).toString('hex'))
            sock.write(Buffer.concat([writeVarInt(inner.length), inner]))
            console.log('[POS] TeleportConfirm sent', body.toString('hex'))
        }
         return
    }
...
function connect() {
    ...
    sock.on('close', (hadError) => {
        log(`🔌 Отключён от сервера ${hadError ? '(с ошибкой)' : ''}`)
        isRunning = false
        sock = null
        stopPositionUpdates()
    })
...
tbot.on('/code', (msg) => {
    ...
    return msg.reply.text(`✅ Отправлена команда: /verify ${code.toUpperCase()}`)
})
...
tbot.on('/cmd', (msg) => {
    ...
    sendCommand(cmd)
    return msg.reply.text(`✅ Выполнено: /${cmd}`)
})
